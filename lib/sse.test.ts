...
import { SSEJSClientTransport } from './sse'
import {
  OAuthClientProvider,
  UnauthorizedError,
...
// mock sse.js with eventsource
jest.mock('sse.js', () => {
  // eslint-disable-next-line import/no-extraneous-dependencies
  const { EventSource } = require('eventsource')
  return {
    SSE: class extends EventSource {
      constructor(
        url: string | URL,
        options: {
        ...
        super(url, {
          ...
          fetch: (
            ...
            (options.fetch || global.fetch)(input, {
              ...init,
              headers: {
                ...init?.headers,
                ...options.headers,
...
describe('SSEJSClientTransport', () => {
  let server: Server
  let transport: SSEJSClientTransport
  let baseUrl: URL
  ...
  describe('connection handling', () => {
    it('establishes SSE connection and receives endpoint', async () => {
      transport = new SSEJSClientTransport(baseUrl)
